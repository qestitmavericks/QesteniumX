...
import com.qestit.enums.Browser;
import com.qestit.enums.CategoryType;
import com.qestit.helpers.CaptureHelpers;
import com.qestit.helpers.FileHelpers;
import com.qestit.helpers.PropertiesHelpers;
import com.qestit.helpers.ScreenRecoderHelpers;
import com.qestit.keywords.WebAction;
import com.qestit.report.AllureManager;
import com.qestit.report.ExtentReportManager;
import com.qestit.report.SlackManager;
import com.qestit.report.TelegramManager;
import com.qestit.utils.BrowserInfoUtils;
import com.qestit.utils.EmailSendUtils;
import com.qestit.utils.JiraUtils;
import com.qestit.utils.LogUtils;
import com.qestit.utils.ZipUtils;
...
import static com.qestit.constants.FrameworkConstants.*;
...
//        }
    ...
    public void onTestFailure(ITestResult iTestResult) {
        ...
        if (SCREENSHOT_FAILED_STEPS.equals(YES)) {
        	screenshotFilePath =CaptureHelpers.captureScreenshot(DriverManager.getDriver(), getTestName(iTestResult));
        }
        ...
        //JIRA Bug Creation
        if (FrameworkConstants.JIRA_CREATION_ISSUE.equalsIgnoreCase("yes")) {
            String issueNUM = null;
            try {
                String exceptionMessage = iTestResult.getThrowable() != null ? iTestResult.getThrowable().getMessage() : "No exception message";
                String stackTrace = JiraUtils.getStackTrace(iTestResult.getThrowable());

                String issueDescription = String.format(
                    "Test case failed: %s\n\nException Message: %s\n\nStack Trace: %s",
                    getTestName(iTestResult), exceptionMessage, stackTrace
                );

                JSONObject adfDescription = JiraUtils.formatDescriptionToADF(issueDescription);
                Map<String, Object> jiraFields = new HashMap<>();
                jiraFields.put("project", Collections.singletonMap("key", "QES"));
                jiraFields.put("summary", "Failure in Test: " + getTestName(iTestResult));
                jiraFields.put("description", adfDescription);
                jiraFields.put("issuetype", Collections.singletonMap("name", "Bug"));
                jiraFields.put("assignee", Collections.singletonMap("id", "609caa4520ad1b0070ecd6bf"));
                jiraFields.put("priority", Collections.singletonMap("id", "2"));
                jiraFields.put("labels", Collections.singletonList("bugfix"));
                jiraFields.put("reporter", Collections.singletonMap("id", "609caa4520ad1b0070ecd6bf"));
                // Convert Map to JSONObject
                JSONObject jsonPayload = JiraUtils.convertMapToJSON(jiraFields);

                // Add additional fields using addField method
//                JiraUtils.addField(jsonPayload, "priority", Collections.singletonMap("id", "2"));
//                JiraUtils.addField(jsonPayload, "labels", Collections.singletonList("bugfix"));
//                JiraUtils.addField(jsonPayload, "reporter", Collections.singletonMap("id", "609caa4520ad1b0070ecd6bf"));
                
                issueNUM = jiraUtils.createJiraIssue(jiraFields);
            ...
            if (issueNUM != null && screenshotFilePath != null) {
                try {
                    jiraUtils.addAttachmentToJiraIssue(issueNUM, screenshotFilePath);
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
