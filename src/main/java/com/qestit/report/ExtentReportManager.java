...
import com.qestit.utils.IconUtils;
...
public class ExtentReportManager {
	...
	public static void initReports() {
		if (Objects.isNull(extentReports)) {
			...
			String reportNameWithLogo = "<img src='https://qestit.com/wp-content/uploads/cropped-Favicon_Questit-270x270.png' "
                    + "style='height:18px; width:auto; margin-right: 5px;' "
                    + "alt='Logo' " 
                    + "title='Project Logo' /> "
                    + FrameworkConstants.REPORT_TITLE;
			spark.config().setTheme(Theme.STANDARD);
			spark.config().setDocumentTitle(FrameworkConstants.REPORT_TITLE);
			spark.config().setReportName(reportNameWithLogo);
	...
	public static void addScreenShot(String message) {
		...
		ExtentTestManager.getExtentTest().log(Status.INFO, MediaEntityBuilder
				.createScreenCaptureFromPath(String.valueOf(CaptureHelpers.getScreenshotFile(message))).build());
	...
	public static void addScreenShot(Status status, String message) {
		...
		ExtentTestManager.getExtentTest().log(status, MediaEntityBuilder
				.createScreenCaptureFromPath(CaptureHelpers.getScreenshotAbsolutePath(message)).build());

	}

	synchronized public static void addAuthors(AuthorType[] authors) {
		if (authors == null) {
			ExtentTestManager.getExtentTest().assignAuthor("qestit");
		} else {
