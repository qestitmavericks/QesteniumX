...
import com.qestit.report.ExtentTestManager;
import com.qestit.utils.BrowserInfoUtils;
import com.qestit.utils.DateUtils;
...
/**
 * Keyword WebAction is a generic class that is a preprocessed library with many custom functions from Selenium and Java.
 * Returns is a Class containing Static functions. Callback is used by taking the class name and dotting the function name (WebAction.method)
 */
public class WebAction {
     ...
     *
     * @param by to be an element of object type By
     * @return text of QR Code
     */
    @Step("Get QR code from image {0}")
    public static String getQRCodeFromImage(By by) {
        String qrCodeURL = WebAction.getAttributeElement(by, "src");
    ...
    /**
     * Retrieves a Selenium By object from a locator key defined in a properties file.
     * Assumes locators are stored with a prefix indicating their type (e.g., "xpath:", "id:", "css:").
     *
     * @param key The key associated with the locator in the properties file.
     * @return By The Selenium By object for the specified locator.
     * @throws IllegalArgumentException for unsupported locator types or incorrect format.
     */
    public static By getLocator(String key) {
        String fullLocator = PropertiesHelpers.getValue(key);
        if (fullLocator == null) {
            throw new IllegalArgumentException("Locator not found for key: " + key);
        }

        String[] parts = fullLocator.split(":", 2);
        if (parts.length != 2) {
            throw new IllegalArgumentException("Incorrect locator format for key: " + key);
        }
    ...
    public static ArrayList getValueTableByColumn(int column) {
        ...
        if (totalRows.size() < 1) {
            ...
            for (int i = 1; i <= totalRows.size(); i++) {
                ...
                LogUtils.info("Row " + i + ":" + title.getText()); //If you don't like logging, then delete it
            }
        }
